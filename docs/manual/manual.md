...
1. Raspberry Pi Zero (or TODO). This manual will assume that you have a Raspberry Pi Zero
2. PC or notebook with a SD card reader
3. SD card with 16 GB memory or larger
...
5a)   Download pre-compiled software image from one the following links:
...
Next confirm that it really this SD card you want to write on
...
```
With the shrunken image repeat the steps with the RaspBerry Pi Imager Software as described aboce.

5d) Insert card into SD card slot of Raspberry Pi.
...
2. **Waiting for Boot:**
   Allow the haxophone a minute to complete the boot process. You will hear a the haxophone startup tune if you have your headphones connected
